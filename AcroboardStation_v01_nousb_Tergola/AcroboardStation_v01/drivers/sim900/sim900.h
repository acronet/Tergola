...
uint16_t sim900_http_get_prepare( const char * const get_URL, const uint8_t isURLPGM );
uint16_t sim900_http_get( const char * const get_URL, const uint8_t isURLPGM, char * const return_value, uint16_t * const rv_len );
...
uint16_t sim900_tcp_send(const char * const pBuf, const uint8_t isBufPGM, const uint16_t dimBufSend, char * const pAnswer, const uint16_t * pdimBufAnsw);
#endif /* SIM900_H_ */