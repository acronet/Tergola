...
//
	...
	//snprintf_P(szBuf+l1,BUFFER_SIZE-l1,PSTR("#TIME=%lu&B=%u&Tmcu=%u&P=%u&S=%u") ,
	//ds.data_timestamp,
	//ds.vBat,
	//ds.tempMCU,
...
uint16_t send_data_with_post( DL_SEND_PARAMS * const pPara )
{
	...
	while(iter<QUEUE_LEN) {
		...
		PSTR("#TIME=%lu&B=%u&T_MCU=%u&Lmean=%d&Lmedian=%d&Lmax=%d&Lmin=%d&Lmean_2=%u") ,
		ds.data_timestamp ,
		ds.vBat,
		ds.tempMCU,
		ds.ls.val,
		...
		post_data_end += strnlen(szBuf+post_data_end,1024);
...
static uint8_t  dl_snapshot_make(const uint32_t ts)
{
	...
	volatile LG_MB7040_STATS * const ls = &p_item->ls;
	MB7040_getStats(ls);
	
	//TODO: insert voltmeter here
...
uint16_t dl_task_store_data( void )
{
	...
	while(idx!=g_data_snapshot_end) {
		...
		pd->tempMCU = temp; //TODO: avoid this trick, the data should be available via the snapshot
